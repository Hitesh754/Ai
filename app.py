...
st.header(" 1. Food Image Analysis (Optional)")
        ...
        if isinstance(meal_plan_data, dict):
            ...
            if not meal_plan_data:
                ...
                for day_key, day_content in sorted_items:
                    ...
                    with col_t1: st.metric("Calories", f"{utils.format_number(daily_calories)} kcal")
                    with col_t2: st.metric("Protein", f"{utils.format_number(daily_protein)} g")
                    with col_t3: st.metric("Carbs", f"{utils.format_number(daily_carbs)} g")
                    with col_t4: st.metric("Fat", f"{utils.format_number(daily_fat)} g")
    ...
    if st.button(" Generate Weekly Grocery List"):
        ...
        if current_meal_plan_data:
            with st.spinner("Analyzing meal plan to create grocery list..."):
                # Assuming gemini_api is also in the same directory or properly imported
